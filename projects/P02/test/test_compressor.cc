...
class Compressor : public ::testing::Test {
 protected:
  void SetUp() override {
    init_test();
  }
...
// Test definitions
TEST(Compressor, test_delta_enc)
{
...
TEST(Compressor, test_delta_dec)
{
...
TEST(Compressor, test_delta_enc_dec)
{
...
TEST(Compressor, test_zigzag_enc)
{
    check_retcode(zigzag_encoding(delta, out_u32, DATA_LEN));

    ASSERT_EQ(true, equal_u32_buffers(out_u32, delta_zigzag, DATA_LEN));
}

TEST(Compressor, test_zigzag_dec)
{
    check_retcode(zigzag_decoding(delta_zigzag, out_i32, DATA_LEN));

    ASSERT_EQ(true, equal_i32_buffers(out_i32, delta, DATA_LEN));
}

TEST(Compressor, test_zigzag_enc_dec)
{
...
TEST(Compressor, test_varint_enc)
{
...
TEST(Compressor, test_varint_dec)
{
...
TEST(Compressor, test_varint_enc_dec)
{
...
TEST(Compressor, test_full_enc_dec)
{
    check_retcode(delta_encoding(original, aux_i32, DATA_LEN));
...
TEST(Compressor, test_signal_compression)
{
    #define MAX_ROW 5000
    #define MAX_COL 1
    ...
    // TODO: make the filepath relative to the project
    readCSV("../projects/P02/test/MLII_10s_360Hz.csv", mat_i32, &rows, &cols, true);
    din_len = rows;
    ...
    PRINTF_DBG("info test_signal_compression\n");
    PRINTF_DBG("  Uncompressed length (bytes): %d\n", unc_len) ;
    PRINTF_DBG("  Compressed length (bytes): %d\n", com_len);   
    PRINTF_DBG("  Compression ratio: %.1f\n", com_ratio);
